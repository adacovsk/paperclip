...
if [ ! -f "$BASE" ] || [ "$(git rev-parse origin/main)" != "$(cat "$BASE")" ]; then
  ...
  if [ "$N" -lt "$FRESHNESS_CAP" ]; then
    ...
    #   {task-id}         the WORK task the worktree is named for (AA-7270) — the
    #                     key every sentinel file is written under.
    #   {verify-task-id}  YOUR OWN assigned task (the `Verify: AA-7270 …` subtask,
    #                     AA-7293). It is what the sentinel aliases are named and
    #                     what the completion callback binds to, so a later wake
    #                     reaches the task that can actually Land. When you are
    #                     assigned the work task directly, the two are the same
    #                     string and the alias loop no-ops by design.
