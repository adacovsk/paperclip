...
rm -f "$VERIFY_DIR/{task-id}.exit" "$VERIFY_DIR/{task-id}.base" "$VERIFY_DIR/{task-id}.freshness" "$VERIFY_DIR/{task-id}.pid" "$VERIFY_DIR/{task-id}.integration" "$VERIFY_DIR/{task-id}.integration.log"   # clear sentinel + base + freshness counter + report-only integration result so a stray re-wake won't re-land or re-report
