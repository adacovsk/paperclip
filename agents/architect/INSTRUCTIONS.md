...
7. **Detached-build liveness — probe `/proc`, never trust a grep.** Deciding "is the detached `verifyrun-{task-id}` build still alive?" via `pgrep -af verifyrun-{id}` (or `ps | grep`) false-negatives intermittently (snapshot race / wrapper interference) — each false negative triggers a wasteful duplicate relaunch that then stacks on the flock. The reliable primitive is a direct pid probe: at launch the wrapper records its own PID into `$VERIFY_DIR/{id}.pid`, then check `test -d /proc/"$(cat "$VERIFY_DIR/{id}.pid")"` (true = alive → exit and wait). Do NOT use `kill -0` (the sandbox denies `kill`). Only relaunch when ALL of: sentinel absent, `{id}` absent from the wrapper census below, AND the log mtime is stale (not ~now).
   ...
   - **The wakeup callback MUST carry `payload.issueId` (`$PAPERCLIP_ISSUE_ID`, the UUID — not the `AA-` identifier). Do not drop it as redundant.** A wake with no `issueId` has nothing to bind to, so the server fills one in from whatever task this agent's *resumed session* was last on. Every sentinel callback then lands on that same task no matter which build finished. Measured: seven verifies completed green between 05:20 and 07:15 (one per slot, ~19 min apart, exactly right), and all fourteen resulting wakes bound to a single task — which re-launched its own build each time while the other six sentinels sat green and unread on disk. Nothing was broken about the builds; the results were simply unaddressable. The identifier is already in scope here (the alias loop below uses `$PAPERCLIP_ISSUE_IDENTIFIER`); the UUID is the one the wake path needs. An empty value is harmless — it degrades to the old behaviour rather than erroring — so the guard is to keep the field, not to test it.
